...
from trainer2 import Trainer
from metric import *
from resnet import CNN2Large,BaselineCNN3
from vit import ViT
...
def group_and_average(predictions):
    # Assuming predictions is a 2D numpy array with shape (m, batch)
    m, batch = predictions.shape

    predictions = predictions.T

    # Sort predictions based on similarity for each sample
    # sorted_predictions = np.sort(predictions, axis=1)

    # Threshold for considering values as similar
    similarity_threshold = 0.1  # You can adjust this threshold based on your needs
    similarity_pred = []
    for preds in predictions:
        ...
        while len(preds) != 0:
            ...
            for index, pred in enumerate(preds):
                if abs(pred - anchor) <= similarity_threshold:
                    near_list.append(pred)
                    delete_list.append(index)
            preds = np.delete(preds, delete_list)
            near_group.append(near_list)
        similarity_pred.append(near_group)

    averaged_predictions = []
    for group in similarity_pred:
        max = 0
        pre_list = []
        for index, near_group in enumerate(group):
            if len(near_group) > max:
                max = len(near_group)
        for index, near_group in enumerate(group):
            if len(near_group) == max:
                pre_list.extend(near_group)
        average = np.mean(np.array(pre_list))
        averaged_predictions.append(average)

    averaged_predictions = np.array(averaged_predictions)

    # # Take the average of each group for each sample
    # averaged_predictions = np.array([np.mean(group, axis=0) for group in grouped_predictions])
...
def main(args_opt):
    ...
    result_test = []
    args_opt.save_test_result = os.path.join(args_opt.path_root, args_opt.exp_name, 'test_result-cnn-rf-1')
    # args_opt.save_test_result = os.path.join(args_opt.path_root, args_opt.exp_name, 'test_result-2-tt')
    ...
    for fold in range(args_opt.folds):
        ...
        # for fold in [3]:
            ...
            enc_module = CNN2Large(in_channels=4)
            # enc_module = BaselineCNN3(in_channels=4)
            ...
            generate_react_spms_std = np.concatenate([generate_lig_spms_std.reshape(3955, 20, 40, 1),
                                                     ...
                                                     axis=3)
            generate_react_spms_std_reshape = generate_react_spms_std.reshape(3955, 20 * 40 * 4)
            # react_spms_std_reshape = react_spms_std.reshape(3955, 40 * 80 * 4)
            # generate_react_spms_std_short = generate_react_spms_std_reshape[:,
            #                        np.where(generate_react_spms_std_reshape.max(axis=0) != generate_react_spms_std_reshape.min(axis=0))[0]]
            ...
            react_spms_std_reshape = react_spms_std.reshape(3955, 20 * 40 * 4)
            # react_spms_std_reshape = react_spms_std.reshape(3955, 40 * 80 * 4)
            # react_spms_std_short = react_spms_std_reshape[:,
            #                        np.where(react_spms_std_reshape.max(axis=0) != react_spms_std_reshape.min(axis=0))[0]]
            ...
            #
            #     # RandomForestRegressor(n_jobs=-1, criterion='absolute_error'),
            ...
            if args_opt.mode == 'eval' or args_opt.resume == True:
                ...
                for tmp_model, tmp_model_name in zip(models, model_names):
                    if tmp_model_name == 'CNN':
                        ...
                        model_predictions.append(pred_list * 0.99)
                    else:
                        save_dir = os.path.join(args_opt.path_root, args_opt.exp_name, tmp_model_name)
                        model_save_path = save_dir + f'/fold_{fold}_run_{i}.joblib'
                        ...
                        pred_list = np.array(pred_list)
                        model_predictions.append(pred_list * 0.01)
                ...
                combined_predictions = np.sum(np.array(model_predictions), axis=0)
                # combined_predictions = group_and_average(np.array(model_predictions))
        ...
        # 保存
        workbook2.save(os.path.join(args_opt.path_root, args_opt.exp_name) + '/prediction-cnn-rf-1.xls')
        # workbook2.save(os.path.join(args_opt.path_root, args_opt.exp_name) + '/prediction-2-tt.xls')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_root', type=str, default='/media/data1/Models_ly/3DChemical/Doyle-11/ensemble-spms-cnn-cat2-channel-20X40-all-1-again/',
    # parser.add_argument('--path_root', type=str, default='/media/data1/Models_ly/3DChemical/Doyle-11/ensemble-spms-cnn-cat2-channel-20X40-all-1/train_10/',
    # parser.add_argument('--path_root', type=str, default='/media/data1/Models_ly/3DChemical/Doyle-11/ensemble-spms-cnn-cat2-channel-20X40-true-1/',
    # parser.add_argument('--path_root', type=str, default='/media/data1/Models_ly/3DChemical/Doyle-11/ensemble-spms-cnn-cat2-channel-20X40-gen-1/',
    # parser.add_argument('--path_root', type=str, default='/media/data1/Models_ly/3DChemical/Doyle-11/ensemble-spms-cnn-20X40-all-1/',
    # parser.add_argument('--path_root', type=str, default='/media/data1/Models_ly/3DChemical/Doyle-11/ensemble-spms-cnn-cat2-20X40-all-1/',
    # parser.add_argument('--path_root', type=str, default='/media/data1/Models_ly/3DChemical/Doyle-11/ensemble-spms-cnn-channel-20X40-all-1/',
